    ...
    <div class="row">
        ...
        <table id="tb-veg" class="uk-table uk-table-divider">
          <thead>
            ...
            <tbody >
              <tr v-for="shake in shakes">
                <td><span>{{shake.name}}</span></td>
                <td><span>{{shake.price }}</span></td>
                <td><button class="uk-button uk-button-secondary" href="#modal-center" uk-toggle v-on:click="setData(shake.name,shake.price,shake.id)">Add to Cart</button></td>
                <td></td>
              </tr>
            </tbody>
        </table>
...
export default {
  ...
  computed:{
    shakes(){
      return this.$store.getters.getShakes
    }
  }
}
</script>
