          ...
          <thead>
            ...
            <tbody >
              <tr v-for="beverage in beverages">
                <td><span>{{beverage.name}}</span></td>
                <td><span>{{beverage.price }}</span></td>
                <td><button class="uk-button uk-button-secondary" href="#modal-center" uk-toggle v-on:click="setData(beverage.name,beverage.price,beverage.id)">Add to Cart</button></td>
                <td></td>
              </tr>
            </tbody>
        </table>
...
export default {
  ...
  computed:{
    beverages(){
      return this.$store.getters.getBeverages
    }
  }
}
</script>
